...
import tools
import examples
#import solution as crank # instructor code, students do not have it.
import proj10 as crank # student code, uncomment this line to test.
...
if __name__ == "__main__":
    board = examples.board_initial()
    board.display()
    
    start = time.time()
    next_move = get_next_move(board, 'black')
    end = time.time()
    print("For black:", next_move)
    print(end - start)
    
    start = time.time()
    next_move = get_next_move(board, 'white')
    end = time.time()
    print("For white:", next_move)
    print(end - start)
    ...
    # ---
    board = examples.board_capture_blacks()
    board.display()
    ...
    # ---
    board = examples.board_capture_blacks2()
    board.display()
